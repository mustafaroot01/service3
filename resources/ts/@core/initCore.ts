...
const _syncAppRtl = () => {
  ...
  const supported = (themeConfig.app.i18n.langConfig ?? []).map(lang => lang.i18nLang)

  // A cookie left over from a language that no longer exists would leave the
  // app in that language's direction forever, so it is dropped on sight.
  if (storedLang.value && !supported.includes(storedLang.value))
    storedLang.value = null

  if (locale.value !== storedLang.value && storedLang.value)
    locale.value = storedLang.value

  if (!storedLang.value)
    locale.value = themeConfig.app.i18n.defaultLocale
...
const initCore = () => {
  ...
  // Direction must be applied even with the language switcher turned off,
  // otherwise an Arabic-only panel renders left to right.
  _syncAppRtl()
}
