...
```
POST /auth/register    →  201، الحساب pending والرمز انطلق  →  شاشة رمز التحقق
POST /auth/verify-otp  →  200، الحساب صار active، بلا توكن  →  شاشة الدخول
POST /auth/login       →  200 + التوكن                      ←  المستخدم داخل
```

**ثلاث خطوات.** الحساب يُنشأ بحالة `pending` ولا يُفتح إلا برمز واتساب، ولا واحدة من الخطوتين الأوليين تُرجع توكناً — الدخول يبقى خطوة مستقلة.
...
> **لا يوجد توكن.** انتقل لشاشة رمز التحقق وابدأ عدّاداً تنازلياً بـ`resend_in`.

### `POST /customer/auth/verify-otp`

**الطلب:** `{ "phone": "07719998877", "code": "123456" }`

**الرد `200` — الحساب صار `active`:**
```json
{
  "success": true,
  "message": "تم توثيق رقمك، سجّل الدخول للمتابعة",
  "data": { "phone": "9647719998877" }
}
```
...
```json
{
  ...
  "errors": { "otp": ["INVALID_CODE"] }
}
```

**رقم موثّق سابقاً `422`:** `errors.phone[0]` = `"هذا الرقم موثّق بالفعل، سجّل الدخول"`.

> ولا توكن هنا أيضاً — انقله لشاشة الدخول وعبّئ له الرقم.
...
زر «لم يصلني الرمز» في **شاشتَي الرمز**. الخادم يختار نوع الرمز من حالة الحساب.
...
|---|---|---|
| `register` | الحساب لم يوثّق بعد | `verify-otp` |
| `reset` | الحساب موثّق | `reset-password` |
...
> رقم بلا حساب: `422` مع `"لا يوجد حساب بهذا الرقم"`. وحساب لم يوثَّق بعد: `422` مع `"لم يتم توثيق رقمك بعد، اطلب رمز التحقق"` — لا استعادة قبل التوثيق.
...
|---|---|---|---|
| `pending` | بانتظار التوثيق | **لحظة التسجيل** — قبل إدخال الرمز | ❌ |
...
| 🌐 | `POST /customer/auth/verify-otp` | توثيق الرقم ← يفتح الحساب |
